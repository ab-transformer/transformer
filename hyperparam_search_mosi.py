...
def train(c):
    subprocess.run(
        f"python train_all.py --project_name report_mosi --lr 0.001 --shuffle --project_dim 50 --layers 4 --num_heads 10 --dataset mosi --batch_size 8 --relu_dropout {c['relu_dropout']} --res_dropout {c['res_dropout']} --embed_dropout {c['embed_dropout']} --attn_dropout {c['attn_dropout']} --attn_dropout_a {c['attn_dropout_a']} --attn_dropout_v {c['attn_dropout_v']} --out_dropout {c['out_dropout']} --num_epochs 100".split(),
