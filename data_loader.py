...
def load_data(
    subset_name: str, batch_size: int, memory_pinning: bool, drop_diff_size: bool = True
):
    ...
    video = np.stack(
        [
            np.expand_dims(np.load(os.path.join(vid_dir, key + ".npy")), axis=0)
            for key in shared_keys
        ]
    )
    print("Loaded video " + str(video.shape))
    text = np.stack(
        [np.load(os.path.join(txt_dir, key + "_bertemd.npy")) for key in shared_keys],
        axis=0,
    )
    ...
    dl = torch.utils.data.DataLoader(
        ds, batch_size=batch_size, shuffle=True, pin_memory=memory_pinning
    )
    return dl
